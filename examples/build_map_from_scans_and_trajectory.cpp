...
std::vector<std::pair<Eigen::Affine3d, unsigned long>> getStampedTransformations(const std::string& filepath) {
    ...
    if (xIndex == -1 || yIndex == -1 || zIndex == -1 || qxIndex == -1 || qyIndex == -1 || qzIndex == -1 || qwIndex == -1 || secIndex == -1 || nsecIndex == -1) {
...
int main(int argc, char* argv[]) {
    ...
    auto mapper = std::make_unique<Mapper>(configYaml, true, false, true, false);
